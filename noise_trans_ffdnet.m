...
addpath('./RTV/')
addpath('./utils/')
addpath('ffdnet\utilities\');
net_flex=load('ffdnet\models\FFDNet_color.mat');
...
for img_i = 6
    ...
    for iter=1:maxiter
        ...
        Nimg2=single(Nimg2);
        sigmas=sigma_target;
        res = vl_simplenn(net_flex.net,Nimg2,[],[],'conserveMemory',true,'mode','test');
        % res = vl_ffdnet_matlab(net_flex.net, Nimg2); % use this if you did not install matconvnet; very slow
        De_img2=res(end).x;
        if(iter==maxiter)
            break;
        end
        ...
        sigmas=max(sigmas,[],3);
        [H,W]=size(sigmas);
        sigmas=imresize(sigmas,[H/2,W/2]);
        img_smooth1=single(img_smooth1);
        res = vl_simplenn(net_flex.net,img_smooth1,[],[],'conserveMemory',true,'mode','test');
        % res = vl_ffdnet_matlab(net_flex.net, img_smooth1); % use this if you did not install matconvnet; very slow
        img_smooth=res(end).x;
    ...
    sigmas=sigma_target;
    res = vl_simplenn(net_flex.net,single(Nimg1),[],[],'conserveMemory',true,'mode','test');
    % res = vl_ffdnet_matlab(net_flex.net, single(Nimg1)); % use this if you did not install matconvnet; very slow
    De_img1=res(end).x;
    De_img1=double(De_img1);
