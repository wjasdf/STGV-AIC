function S=S_estimate(im,alpha1,sigma)
if (~exist('sigma','var'))
    sigma=3;
end
if(size(im,3)==1)
    [wx,wy]=computeTextureWeights(im(:,:,1),sigma);
    S(:,:,1)=solveLinearEquation(im(:,:,1),wx,wy,alpha1);
else
[wx,wy]=computeTextureWeights(im(:,:,1),sigma);
S(:,:,1)=solveLinearEquation(im(:,:,1),wx,wy,alpha1);

[wx,wy]=computeTextureWeights(im(:,:,2),sigma);
S(:,:,2)=solveLinearEquation(im(:,:,2),wx,wy,alpha1);

[wx,wy]=computeTextureWeights(im(:,:,3),sigma);
...
function ret=conv2_sep(im,sigma)%高斯卷积
